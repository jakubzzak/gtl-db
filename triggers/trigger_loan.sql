create trigger insert_loan_instead on loan instead of insert as
begin
    declare @available_copies int, @is_loanable bit, @can_borrow bit, @books_borrowed int, @customer_type varchar(20);
    select @available_copies = available_copies, @is_loanable = is_loanable from book join inserted on isbn = inserted.book_isbn;
    select @can_borrow = can_borrow, @books_borrowed = books_borrowed, @customer_type = type from customer join inserted on inserted.customer_ssn = customer.ssn;
    if (@is_loanable = 0)
...
go
create trigger update_loan_instead on loan instead of update as
    begin
